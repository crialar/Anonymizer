...
def _qa_severity_badge(severity: str, count: int = None) -> str:
    ...
    Two severities only (Task #58): 🔴 High / 🟡 Low.
    - Per-card calls pass ``count=None`` → bare emoji (compact).
    - Summary-line calls pass an integer ``count`` → classic colored
      rectangle pill with "HIGH · N" / "LOW · N" (same look as the
      pre-#58 Error/Warning badges, just with the new labels).
    """
    emoji = {"high": "🔴", "low": "🟡"}.get(severity, "•")
    label = {"high": "High", "low": "Low"}.get(severity, severity.capitalize())
    if count is None:
        return (f'<span title="{label}" '
                f'style="font-size:1.1rem;line-height:1;">{emoji}</span>')
    colors = {"high": "#c0392b", "low": "#e67e22"}
    return (f'<span style="background:{colors.get(severity, "#888")};color:#fff;'
            ...
            f'letter-spacing:0.02em;">{label.upper()} · {count}</span>')
...
def _render_qa_check_tab():
    ...
    if summary["total"] == 0 and overrides_pending == 0:
        ...
        # rest of the time. High/Low badges always show (headline numbers).
        extras_html = ""
        ...
        st.markdown(
            f"**Summary:** {_qa_severity_badge('high', summary['high'])} &nbsp; "
            f"{_qa_severity_badge('low', summary['low'])}{extras_html}",
            unsafe_allow_html=True,
        )
    ...
    for idx, (cid, cat) in enumerate(cat_items):
        ...
        with st.expander(header, expanded=False):
            ...
            for issue_idx, issue in enumerate(visible_issues):
                ...
                # so the reviewer sees both segments together. Diff is
                # painted on the cell whose axis diverges.
                if is_inconsist:
                    ...
                    def _stack_inconsist(ref_html: str, this_html: str) -> str:
                        return (
                            ...
                            f'<div style="font-size:0.74rem;color:#5e5f6b;'
                            f'font-weight:600;margin-bottom:2px;">This · seg {seg_id}</div>'
                            f'<div>{this_html}</div>'
                        )
