...
def _text_contains_glossary_term(text: str, term: str,
                                   ...
                                   term_stem_cache: Optional[Dict[str, set]] = None,
    ...
    ``term_stem_cache`` (Task #75) memoizes ``term -> stems`` per run so
    the glossary term itself is stemmed once per run instead of once per
    (term × segment). ``text_fold`` (Task #75) is the pre-folded text for
    the literal-fallback substring prefilter; the stem path takes no
    prefilter (an inflected token need not contain the term literally).
    """
    ...
    if term_stem_cache is not None:
        target_stems = term_stem_cache.get(term)
        if target_stems is None:
            target_stems = _word_stems(term, dictionary)
            term_stem_cache[term] = target_stems
    else:
        target_stems = _word_stems(term, dictionary)
...
def _check_glossary_violation(seg, cfg, ctx):
    ...
    if inflected:
        ...
        # Term -> stems memoization is per-RUN (ctx is rebuilt on every
        # run_qa_checks call and the per-side dictionaries are constant
        # within a run; the key is the exact glossary-term string, so no
        # casing collision is possible): each glossary term is stemmed
        # once per run instead of once per (term × segment). Task #75.
        if ctx is not None:
            src_term_stems = ctx.setdefault("_gloss_term_stems_src", {})
            tgt_term_stems = ctx.setdefault("_gloss_term_stems_tgt", {})
        else:
            src_term_stems, tgt_term_stems = {}, {}
    else:
        src_stem_cache = tgt_stem_cache = None
        src_term_stems = tgt_term_stems = None
    # First-token index (literal mode only): from the segment's folded
    # token set, look up which glossary entries can POSSIBLY match — a
    # 5000-term glossary shrinks to the handful whose first word actually
    # appears in the segment. Inflected mode is exempt: the term's first
    # word may appear only as an inflection ("paciente" vs "pacientes"),
    # so the literal token lookup would wrongly discard real candidates.
    if not inflected:
    ...
    for entry in entries_iter:
        ...
        if inflected:
            ...
            if gs and (gs not in src_pre if case_sensitive
        ...
        if inflected:
            in_tgt = _text_contains_glossary_term(tgt_text, gt, tgt_dict,
                                                  tgt_stem_cache,
                                                  tgt_term_stems, tgt_fold)
        elif gt and (gt not in tgt_pre if case_sensitive
