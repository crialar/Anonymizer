...
# rank = more critical. Used by the HTML report's "Sort by severity" toggle
# to order High issues from most to least urgent. NOT shown to the user.
# Tiers (per pharma/clinical reviewer priorities):
#   1 = Critical (file cannot be delivered: empty/untranslated, broken tags,
#       inconsistent translation across segments).
#   2 = Clinical / patient-safety data (numbers, units, dates, IDs, URLs,
#       emails, placeholders, operator/currency symbols).
#   3 = Terminology (client glossary / forbidden terms).
#   4 = Content & coherence (everything else in the Content group).
...
HIGH_PRIORITY_RANK: Dict[str, int] = {
    ...
    # Tier 2 — Clinical safety / data integrity
    "number_mismatch": 2,
    "unit_mismatch": 2,
    "date_format_mismatch": 2,
    "alphanum_id_mismatch": 2,
    "placeholders": 2,
    "urls": 2,
    "emails": 2,
    "symbol_mismatch": 2,
    # Tier 3 — Terminology
    "forbidden_terms": 3,
    "glossary_violation": 3,
    # Tier 4 — Content & coherence
    "mixed_scripts": 4,
    ...
    "repeated_words": 4,
    "first_letter_case": 4,
    "final_punctuation_mismatch": 4,
    "double_spaces": 4,
    "whitespace_edges": 4,
}

CATEGORY_METADATA: Dict[str, Dict[str, Any]] = {
    # Content (Task #58 — absorbs former Structural / Punctuation /
    # Whitespace groups; all 13 checks share the Content red highlight).
    ...
    # Structural — folded into Content (Task #58).
    "bracket_balance":      {"label": "Bracket / quote balance",       "icon": "✏️", "group": "Content",          "severity": SEVERITY_HIGH},
    # Tags & inline elements
    "tag_count":            {"label": "Tag count mismatch",            "icon": "🏷️", "group": "Tags & inline elements", "severity": SEVERITY_HIGH},
    "tag_malformed":        {"label": "Broken / malformed tags",       "icon": "🏷️", "group": "Tags & inline elements", "severity": SEVERITY_HIGH},
    # Special elements (exact match between source & target)
    "urls":                 {"label": "URLs missing or altered",       "icon": "🔗", "group": "Special elements", "severity": SEVERITY_HIGH},
    "emails":               {"label": "Emails missing or altered",     "icon": "🔗", "group": "Special elements", "severity": SEVERITY_HIGH},
    "placeholders":         {"label": "Placeholders / variables",      "icon": "🔗", "group": "Special elements", "severity": SEVERITY_HIGH},
    "number_mismatch":      {"label": "Numbers differ source / target","icon": "🔗", "group": "Special elements", "severity": SEVERITY_HIGH},
    "unit_mismatch":        {"label": "Unit of measurement differs",   "icon": "🔗", "group": "Special elements", "severity": SEVERITY_HIGH},
    "number_format_mismatch": {"label": "Number format wrong for locale","icon": "🔗", "group": "Special elements", "severity": SEVERITY_LOW},
    "date_format_mismatch": {"label": "Date order differs (DD/MM swap)","icon": "🔗", "group": "Special elements", "severity": SEVERITY_HIGH},
    # Punctuation — folded into Content (Task #58).
    ...
    # Whitespace — folded into Content (Task #58). Auto-fixable, Low
    # severity (was Info pre-#58, dropped along with the Info bucket).
    ...
    "alphanum_id_mismatch": {"label": "Alphanumeric IDs differ",       "icon": "🔗", "group": "Special elements", "severity": SEVERITY_HIGH},
    "tag_order":            {"label": "Tag order mismatch",            "icon": "🏷️", "group": "Tags & inline elements", "severity": SEVERITY_HIGH},
    ...
    "symbol_mismatch":      {"label": "Symbol mismatch (operator / currency)", "icon": "🔗", "group": "Special elements", "severity": SEVERITY_HIGH},
...
# groups (Task #58 collapsed Structural/Punctuation/Whitespace into
# Content), one color each — every check inherits its group's color.
    ...
    if fmt == "html":
        ...
        def esc_attr(s: str) -> str:
            ...
            return (esc(s)
                    .replace('"', "&quot;")
                    .replace("'", "&#39;"))
        sev_color = {"high": "#c0392b", "low": "#e67e22"}
        sev_emoji = {"high": "🔴", "low": "🟡"}
        ...
        toolbar_html = (
            ...
            # Default ON — Task #64. Button ships in the active (red) state
            # so reviewers see High issues at the top from the first paint.
            # Click toggles back to the original DOM order (idle white-with-
            # red-border style).
            '<button type="button" data-qa-action="sort" id="qa-sort-btn" '
            'aria-pressed="true" '
            'title="Group rows by severity (🔴 High first, then 🟡 Low)" '
        ...
        totals_line = (
            f"<p><strong>Totals:</strong> 🔴 {s.get('high', 0)} High, "
            f"🟡 {s.get('low', 0)} Low"
        )
        if edited_segs:
            totals_line += (
        ...
        # Task #58: dropped the per-row Category column (redundant with
        # Group, which already drives color + filtering). 6 columns now,
        # widths re-balanced to total 100%: Seg 4 / Sev 5 / Grp 8 / Msg 18
        # / Src 31 / Tgt 34. Source/Target take the lion's share since
        # that's where highlighted spans live.
        parts.append(
            "<table id='qa-table'>"
            "<colgroup>"
            "<col style='width:4%'/>"   # Segment (narrow — only id + ctx icon)
            "<col style='width:3%'/>"   # Severity (narrow — emoji only)
            "<col style='width:8%'/>"   # Group
            "<col style='width:18%'/>"  # Message
            "<col style='width:32%'/>"  # Source
            "<col style='width:35%'/>"  # Target
            "</colgroup>"
            "<thead><tr><th>Segment</th><th>Level</th>"
        ...
        for idx, r in enumerate(rows):
            ...
            if (cid == "inconsistent_translation"
                ...
                # Task #55 — stacked "Ref · seg X" / "This · seg Y" block
                # so the reviewer can read both segments side-by-side.
                # Diff is painted on the cell whose axis diverges:
                #   axis=target → diff on target cell only
                #   axis=source → diff on source cell only
                #   axis=both   → diff on both cells
                ...
                else:  # "target" (default / back-compat)
                ...
                def _stack(ref_html: str, this_html: str) -> str:
                    return (
                        ...
                        f'font-weight:600;margin-bottom:2px;">Ref · seg {ref_id}</div>'
                        f'<div style="margin-bottom:6px;">{ref_html}</div>'
                        f'<div style="font-size:0.72rem;color:#5e5f6b;'
                        f'font-weight:600;margin-bottom:2px;">This · seg {r["segment"]}</div>'
                        f'<div>{this_html}</div>'
                        f'</div>'
                    )
            ...
            parts.append(
                f"<tr class='qa-row' data-row='{idx}' data-segment='{r['segment']}' data-group=\"{esc(r['group'])}\" data-severity='{esc(r['severity'])}' data-priority='{HIGH_PRIORITY_RANK.get(r.get('category_id', ''), 99)}'>"
                f"<td>{seg_cell}</td>"
                f"<td><span class='sev qa-sev-{esc(r['severity'])}' "
                f"title='{esc(r['severity']).capitalize()}' "
                f"style='background:transparent;font-size:1.1rem;'>{emoji}</span></td>"
                f"<td>{esc(r['group'])}</td>"
                f"<td>{esc(r['message'])}"
...
(function(){
  ...
  // true = qa-rows re-grouped High -> Low, preserving the original
  // order within each severity block. Context rows follow their parent.
  // Task #64 — default ON so High issues lead from first paint.
  ...
  var origOrder = [];
  var SEV_RANK = {high: 0, low: 1};

  // Indices of the Source / Target cells inside each row (Seg, Sev,
  // Grp, Msg, Src, Tgt — Category column dropped in Task #58).
  var SRC_IDX = 4, TGT_IDX = 5;
  ...
  function reorder(){
    ...
    if (sortBySeverity) {
      seq.sort(function(a, b){
        ...
        // Task #63 — within the High bucket, sub-sort by internal
        // priority rank (data-priority: 1=Critical → 4=Content). Not
        // surfaced visually; pure ordering hint. Low bucket rows all
        // carry priority=99 so this is a no-op there.
        var pa = parseInt(a.getAttribute('data-priority'), 10);
        var pb = parseInt(b.getAttribute('data-priority'), 10);
        if (isNaN(pa)) pa = 99;
        if (isNaN(pb)) pb = 99;
        if (pa !== pb) return pa - pb;
        // Stable within (severity, priority): keep original DOM order.
  ...
  document.addEventListener('click', function(ev){
    ...
    if (act === 'ctx') {
      ...
      if (!flagged && !before.length && !after.length) {
        ...
        notice.innerHTML = "<td colspan='6' class='qa-ctx-cell' "
      ...
      ctxTr.innerHTML = "<td colspan='6' class='qa-ctx-cell'>" + inner + "</td>";
    ...
    if (act === 'ignore') {
      ...
      if (sb) {
        ...
        if (sortBySeverity) {
          sb.style.background = '#c0392b';
          sb.style.color = '#fff';
          sb.style.border = '1px solid #c0392b';
          sb.textContent = 'Sort by severity ✓';
        } else {
          sb.style.background = '#fff';
          sb.style.color = '#c0392b';
          sb.style.border = '1px solid #c0392b';
          sb.textContent = 'Sort by severity';
        }
      }
  ...
  snapshot();
  // Task #64 — default-ON sort: apply the initial reorder so the first
  // paint matches the button's pressed state.
  if (sortBySeverity) reorder();
  apply();
})();
</script>
"""
