...
def _serialize_inline_tags(element) -> str:
    """Return the element's inner content with inline tags rendered as
    readable XML markers (``<ph id="1"/>``, ``<g id="2">inner</g>``,
    ``<bpt id="1"/>``, ``<ept id="1"/>``, …). Non-inline children fall
    through to their text content.

    Used by the HTML report's per-row "Show tags" viewer (Tags group)
    so the reviewer can see WHICH inline tag is present where the
    cleaned ``source_text`` / ``target_text`` otherwise hides it. The
    output is plain text — the HTML report escapes it before display
    and re-highlights ``<tag …/>`` snippets via the toolbar JS.
    """
    if element is None:
        return ""
    parts: List[str] = []
    ...
    for child in element:
        ...
        if tag in INLINE_TAG_NAMES:
            ...
            inner = _serialize_inline_tags(child)
            if inner:
                parts.append(f"<{tag}{id_attr}>{inner}</{tag}>")
            else:
                parts.append(f"<{tag}{id_attr}/>")
        else:
            # Non-inline / unknown wrapper: just recurse to keep any
            # nested inline tags discoverable. Visible text already
            # surfaces via the recursive ``element.text`` / ``child.tail``.
            inner = _serialize_inline_tags(child)
            if inner:
                parts.append(inner)
        if child.tail:
    ...
    if fmt == "tmx":
        ...
        for idx, tu in enumerate(tree.xpath("//tu"), start=1):
            ...
            segments.append({
                ...
                # Visible text WITH inline tags rendered as readable
                # markers — only consumed by the HTML report's per-row
                # "Show tags" viewer (Tags-group rows). Empty when there
                # are no inline tags so the embedded JSON stays small.
                "source_inline": _serialize_inline_tags(src_seg),
                "target_inline": _serialize_inline_tags(tgt_seg),
        ...
        for idx, tu in enumerate(tree.xpath("//xliff:trans-unit", namespaces=XLIFF_NS), start=1):
            ...
            segments.append({
                ...
                "source_inline": _serialize_inline_tags(src),
                "target_inline": _serialize_inline_tags(tgt),
                "source_element": src,
                "target_element": tgt,
            })
...
def run_qa_checks(xml_content: bytes, filename: str,
    ...
    segments_data = []
    for s in segments:
        src_text = s.get("source_text") or ""
        tgt_text = s.get("target_text") or ""
        entry = {"id": s["id"], "source": src_text, "target": tgt_text}
        # Only embed the inline-tag rendering when it actually differs
        # from the cleaned text (i.e. the segment has at least one
        # inline tag). Keeps the embedded JSON small for tag-free files.
        src_inline = s.get("source_inline") or ""
        tgt_inline = s.get("target_inline") or ""
        if src_inline and src_inline != src_text:
            entry["source_inline"] = src_inline
        if tgt_inline and tgt_inline != tgt_text:
            entry["target_inline"] = tgt_inline
...
def export_qa_report(results: Dict[str, Any], fmt: str = "csv", *,
    ...
    if fmt == "html":
        ...
        parts = [
            ...
            # Per-row "Show tags" button (Tags-group rows only). Uses the
            # Tags palette tan/brown so it visually belongs to that group;
            # active state inverts colors like .qa-ctx-active does for
            # the context button. Sits to the right of the context icon.
            ".qa-tags-btn{background:#fff;border:1px solid #bcbdbe;"
            "border-radius:4px;padding:0;margin-left:4px;cursor:pointer;"
            "line-height:0;font-family:inherit;vertical-align:middle;"
            "display:inline-flex;align-items:center;justify-content:center;"
            "width:22px;height:18px;font-size:10px;font-weight:700;"
            "color:#6b4423;letter-spacing:-0.5px;"
            "transition:background .15s,border-color .15s,box-shadow .15s;}",
            ".qa-tags-btn:hover{border-color:#6b4423;"
            "box-shadow:0 1px 3px rgba(107,68,35,0.25);}",
            ".qa-tags-btn.qa-tags-active{background:#e8d4b8;border-color:#6b4423;}",
            ".qa-tags-row > td.qa-tags-cell{background:#fdf6ed;padding:8px 12px;"
            "border-left:3px solid #6b4423;}",
            ".qa-tags-cell .qa-tags-line{font-family:ui-monospace,SFMono-Regular,"
            "Menlo,Consolas,monospace;font-size:0.85rem;line-height:1.55;"
            "margin-bottom:4px;word-wrap:break-word;overflow-wrap:anywhere;}",
            ".qa-tags-cell .qa-tags-line:last-child{margin-bottom:0;}",
            ".qa-tags-cell .qa-tags-label{display:inline-block;min-width:56px;"
            "font-family:inherit;font-weight:600;color:#6b4423;margin-right:8px;"
            "font-size:0.75rem;text-transform:uppercase;letter-spacing:0.5px;}",
            ".qa-tags-cell .qa-tag-marker{background:#e8d4b8;color:#6b4423;"
            "font-weight:600;padding:0 3px;border-radius:2px;}",
            ".qa-tags-cell .qa-tags-empty{font-style:italic;color:#8a7660;}",
            ...
            "@media print{#qa-toolbar,#qa-help,#qa-clear-groups-btn,.qa-ctx-btn,.qa-tags-btn{display:none !important;} "
        ...
        for idx, r in enumerate(rows):
            ...
            # Tags-group rows get an additional viewer button that
            # expands a sibling row showing source + target WITH
            # inline tag markers (<ph id="1"/>, <g id="2">…</g>,
            # <bpt id="1"/>, <ept id="1"/>). The cleaned source_text /
            # target_text stripped these tags out for the other QA
            # checks, so without this viewer the reviewer would see
            # "source has 0, target has 1" with no visible tag.
            if r.get("group") == "Tags":
                seg_cell += (
                    f"<button type='button' class='qa-tags-btn' "
                    f"data-qa-action='tags' data-row='{idx}' "
                    f"data-segment='{r['segment']}' "
                    f"aria-label='Show inline tags in source and target' "
                    f"title='Show inline tags'>&lt;/&gt;</button>"
                )
            ...
            if (cid == "inconsistent_translation"
                ...
                # reviewer reads both segments side-by-side. Top row is the
                # reference (lower seg_id); Task #66 dropped the "Ref ·" /
                # "This ·" prefixes — the stack itself conveys which is ref.
            ...
            parts.append(
                f"<tr class='qa-row' data-row='{idx}' data-segment='{r['segment']}' data-group=\"{esc(r['group'])}\" data-group-rank='{GROUP_RANK.get(r['group'], 99)}' data-cat-rank='{CATEGORY_RANK.get(r.get('category_id', ''), 999)}'>"
                f"<td>{seg_cell}</td>"
...
(function(){
  ...
  function reorder(){
    ...
    seq.forEach(function(tr){
      ...
      if (key) {
        ...
        var tagsR = tbody.querySelector(
          'tr.qa-tags-row[data-tags-of="' + key + '"]');
        if (tagsR) tbody.appendChild(tagsR);
      }
    });
  }
  ...
  function apply(){
    ...
    rows().forEach(function(tr){
      ...
      if (key) {
        ...
        var tagsRow = document.querySelector(
          'tr.qa-tags-row[data-tags-of="' + key + '"]');
        if (tagsRow) tagsRow.style.display = hide ? 'none' : '';
      }
      if (!hide) visible++;
    });
  ...
  document.addEventListener('click', function(ev){
    ...
    if (act === 'tags') {
      // Toggle the per-row inline-tags viewer for Tags-group rows.
      // Renders source + target with the inline tag markers
      // (<ph id="1"/>, <g id="2">…</g>, <bpt id="1"/>, <ept id="1"/>)
      // baked in. Falls back to the cleaned source/target text when
      // the segment has no inline tags (defensive — the button only
      // renders for Tags-group rows, but the JSON payload omits
      // source_inline/target_inline when they would equal the cleaned
      // text, so the empty case is real).
      ev.preventDefault();
      var rowIdxT = t.getAttribute('data-row');
      var segIdT = parseInt(t.getAttribute('data-segment') || '0', 10);
      if (!rowIdxT || !segIdT) return;
      var existingT = document.querySelector(
        'tr.qa-tags-row[data-tags-of="' + rowIdxT + '"]');
      if (existingT) {
        existingT.parentNode.removeChild(existingT);
        t.classList.remove('qa-tags-active');
        return;
      }
      var segT = SEG_BY_ID[segIdT] || null;
      var srcLine = (segT && segT.source_inline) || (segT && segT.source) || '';
      var tgtLine = (segT && segT.target_inline) || (segT && segT.target) || '';
      // Highlight inline tag markers (`<name … />` / `<name …>` /
      // `</name>`) inside the rendered line. Names limited to the
      // 10 XLIFF/TMX inline tags so prose like "a < 5" never paints.
      // Attribute body must tolerate escaped entities (`&quot;`, `&amp;`,
      // numeric refs) since the surrounding text was passed through
      // escHtml() — so `id="1"` becomes `id=&quot;1&quot;`. Earlier
      // pattern `[^&]*?` broke on the very first `&quot;` and skipped
      // painting the marker for the common attribute case.
      var TAG_RE = /&lt;\/?(?:ph|bpt|ept|it|bx|ex|x|g|mrk|sub)(?:\s(?:[^&<>]|&(?:[a-zA-Z]+|#\d+);)*?)?\/?&gt;/g;
      function paintTags(s){
        return escHtml(s).replace(TAG_RE, function(m){
          return '<span class="qa-tag-marker">' + m + '</span>';
        });
      }
      var hasAny = !!(srcLine || tgtLine);
      var inner = '';
      if (!hasAny) {
        inner = '<div class="qa-tags-empty">No inline tag content '
          + 'available for this segment.</div>';
      } else {
        inner = '<div class="qa-tags-line">'
          + '<span class="qa-tags-label">Source</span>'
          + paintTags(srcLine) + '</div>'
          + '<div class="qa-tags-line">'
          + '<span class="qa-tags-label">Target</span>'
          + paintTags(tgtLine) + '</div>';
      }
      var tagsTr = document.createElement('tr');
      tagsTr.className = 'qa-tags-row';
      tagsTr.setAttribute('data-tags-of', rowIdxT);
      tagsTr.innerHTML = "<td colspan='5' class='qa-tags-cell'>" + inner + "</td>";
      var hostRowT = document.querySelector(
        'tr.qa-row[data-row="' + rowIdxT + '"]');
      if (hostRowT && hostRowT.parentNode) {
        // Insert AFTER any context row already attached to this same
        // row index so both viewers can be open at once without the
        // tags block jumping above the context.
        var anchor = hostRowT.nextSibling;
        while (anchor && anchor.nodeType === 1 &&
               anchor.className === 'qa-context-row' &&
               anchor.getAttribute('data-context-of') === rowIdxT) {
          anchor = anchor.nextSibling;
        }
        hostRowT.parentNode.insertBefore(tagsTr, anchor);
        t.classList.add('qa-tags-active');
      }
      return;
    }
